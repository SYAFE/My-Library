    /*********************************************
        RUM Performance PSI Measures Process        
        Process : S28_1101_rq.js
        Writer  : TY
        Date    : 2022-02-22
    **********************************************/
    const   API_PARAMETER = "https://www.googleapis.com/pagespeedonline/v5/runPagespeed?url=";
    const   API_KEY = "AIzaSyDtGlLknlGFVQXwDkYws0rze5H2EwT2B9Q";
    // const   API_KEY = "AIzaSyD5M_7Bay3bB1Xzfytf-xSPCeXLFaDHftg";
    ...
    const filters = [
        ...
        ['JsonBody','loadingExperience','metrics','INTERACTION_TO_NEXT_PAINT','percentile'],
    ...
    function Call_API(arrTarget)
    {
     ...
     Promise.all(arrAPITarget.map(Call_API)).then(arrResult => {
         arrResult.forEach((iCount, i) => {
            ...
            if(iCount < LOOP_COUNT){
                var row = { "url" : `${arrUrlList[i]}`, "flag" : "0"};
                arrRetry.push(row);
            }
            else{
                var row = { "url" : `${arrUrlList[i]}`, "flag" : "1"};
                arrRetry.push(row);
            }
         });
     }).then( () => {
        const objRetry = JSON.stringify(arrRetry);
        fs.writeFileSync(process.argv[2], objRetry);
     });
